resource "local_file" "exportbackend-to-services-mailforwarder-sns-emailforwarder" {
    content  = <<EOF
data "terraform_remote_state" "storage-ses-email" {
   ...
   config = {
    ...
    EOF
    filename = "../../services/emailforwarder/sns-emailforwarder/backend-exported-from-storage-storage-emailforwarder.tf"
