resource "local_file" "exportbackend-to-global-roles-emailforwarder" {
    content  = <<EOF
data "terraform_remote_state" "storage-ses-email" {
   ...
   config = {
    ...
    EOF
    filename = "../../global/roles/lambda-emailforwarder/backend-exported-from-storage-storage-emailforwarder.tf"
