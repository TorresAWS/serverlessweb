resource "local_file" "general-roles-lambda-send-email-from-s3-role" {
    content  = <<EOF
data "terraform_remote_state" "lambda-send-email-from-s3-role" {
   ...
   config = {
    ...
    EOF
    filename = "../../../services/emailforwarder/lambda-emailforwarder/backend-exported-from-global-roles-lambda-send-email-role.tf"
}
