resource "local_file" "exportbackend-to-testlambda" {
    content  = <<EOF
data "terraform_remote_state" "lambda-emailforwarder" {
   ...
   config = {
    ...
    EOF
    filename = "../../../services/emailforwarder/sns-emailforwarder/backend-exported-from-services-emailforwarder-lambda-emailforwarder.tf"
